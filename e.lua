local Success, Error = pcall(function()
	...
	local keys_lol = {}
	InputService.InputBegan:Connect(function(inputObject, g)
		if inputObject.UserInputType == Enum.UserInputType.Keyboard and not InputService:GetFocusedTextBox() then
			for i, v in next, keys_lol do
				if v[1] == inputObject.KeyCode then
					v[2]()
				end
			end
		end
	...
	local IsA = game.IsA
	local FindFirstChild, FindFirstChildWhichIsA, WaitForChild = game.FindFirstChild,game.FindFirstChildWhichIsA,game.WaitForChild

	local Tfind, sort, concat, pack, unpack;
	do
		Tfind, sort, concat, pack, unpack = table.find, table.sort,table.concat,table.pack,table.unpack
	end

	local lower, Sfind, split, sub, format, len, match, gmatch, gsub, byte;
	do
		lower, Sfind, split, sub, format, len, match, gmatch, gsub, byte = string.lower,string.find,string.split, string.sub,string.format,string.len,string.match,string.gmatch,string.gsub,string.byte
	end

	local random, floor, round, abs, atan, cos, sin, rad;
	do
		random, floor, round, abs, atan, cos, sin, rad, clamp = math.random,math.floor,math.round,math.abs,math.atan,math.cos,math.sin,math.rad,math.clamp
	end
	...
	local UIElements = game:GetObjects("rbxassetid://10726102730")[1]
	...
	UILibrary.new = function(ColorTheme)
		...
		UI.Name = game:GetService("HttpService"):GenerateGUID(false):gsub("-", ""):lower()
		syn.protect_gui(UI)
	...
	local NotificationBar = Clone(GuiObjects.Load.NotificationBar)
	local NotificationBar_Parent = Instance.new('ScreenGui')
	NotificationBar_Parent.Name = game:GetService("HttpService"):GenerateGUID(false):gsub("-", ""):lower()
	NotificationBar.Parent = NotificationBar_Parent
	syn.protect_gui(NotificationBar_Parent)
	game:GetService("CoreGui"):WaitForChild("RobloxGui", math.huge)
	NotificationBar_Parent.Parent = game:GetService("CoreGui")["RobloxGui"]

	UILibrary.OldTheme = table.create(1, Colors)[1]

	UILibrary.Notify = function(Title, Message, Time)
		local Notification = GuiObjects.Elements.Notification
		local Clone = Clone(Notification)

		Clone.Message.Text = Message
		Clone.Title.Text = Title or "Notification"
		Clone.Title.TextColor3 = yea
		...
		coroutine.wrap(function()
			local Tween = Utils.TweenAllTransToObject(Clone, 0, Notification)
	...
	function UILibrary:LoadWindow(Title, Size, Theme, autoIdle)
		Colors = Theme or Colors
		...
		Window.Main.Title.TextXAlignment = Enum.TextXAlignment.Left
		...
		WindowLibrary.GetUI = function()
			return self.UI
		end
		WindowLibrary.GetWindow = function()
			return Window
		end
		...
		WindowLibrary.SetSize = function(NewSiz)
			Window.Size = NewSiz
		end
		...
		function WindowLibrary.NewPage(Title)
			...
			function PageLibrary.NewSection(Title)
				...
				local function ToggleFunction(Container, Enabled, Callback, DisabledKeybind, Title, Keybind) -- fpr color picker
					...
					old_title = table.create(1, Title)[1]
					bruh = false
				...
				function ElementLibrary.Button(Text, Callback, DisabledKeybind, keybind)
					...
					local key, old_title, binding, bruh;
					old_title = table.create(1, Text)[1]
					bruh = false

					Toggle.Title.Text = Text
					Toggle.Parent = Section.Options

					AddConnection(CConnect(Toggle.Title.MouseButton1Click, Callback));
					...
					return {changeText = function(Text)
						...
						if key then
							Toggle.Title.Text = Text .. " [".. tostring(keys[key] or key):gsub("Enum.KeyCode.", "") .."]"
						end
					end}
				end

				function ElementLibrary.Text(Text)
...
if Success then return Error end
